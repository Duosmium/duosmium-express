...
async function getAllFirstLetters() {
  if (redisClient) {
    ...
    if (cachedLength > 0) {
      return await redisClient.ZRANGE("schoolLetters", 0, cachedLength - 1, "BYLEX");
    }
  }
...
async function getSchoolRankings(name, city, state, country) {
  ...
  if (redisClient) {
    ...
    if (savedInCache) {
      const output = {};
      for (const tournament of tournaments) {
        output[tournament] = (await redisClient.ZRANGE(`${prefix}:${tournament}`, 0, -1)).map(r => Number(r));
      }
      return output;
    }
  }
  ...
  if (redisClient) {
    for (const tournament of Object.keys(output)) {
      ...
      for (const rank of output[tournament]) {
        await redisClient.ZADD(`${prefix}:${tournament}`, {score: rank.toString(), value: rank.toString()});
      }
    }
  }
  return output;
...
async function getAllRankingsByLetter(letter) {
  ...
  if (redisClient) {
    const schools = await redisClient.ZRANGE(`schools:letters:${letter}`, 0, -1);
    if (schools.length > 0) {
      const output = {}
  ...
  })).map(team => formatSchool(team.name, team.city, team.state, team.country)).sort();
  const output = {};
  for (const team of allTeams) {
    if (!(team in output)) {
  ...
  if (redisClient) {
    for (const team of allTeams) {
      await redisClient.ZADD(`schools:letters:${letter}`, {score: "1.0", value: team});
    }
  }
...
function unformatSchool(name) {
  ...
  return [matches[1], matches[3] ?? "", matches[4] in STATES_BY_POSTAL_CODE ? matches[4] : "", matches[4] in STATES_BY_POSTAL_CODE ? "United States" : matches[4]];
}

module.exports = {getTeam, getAllFirstLetters, addTeam, deleteTeam, deleteAllTeams, getFirstLetter, getTournamentsPerSchool, createTeamDataInput, getSchoolRankingsCombinedName, getAllRankingsByLetter}