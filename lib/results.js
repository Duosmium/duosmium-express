...
async function addResult(resultData) {
  if (redisClient) {
    ...
    await redisClient.DEL(`seasons:${resultData.tournament.year}:${resultData["duosmium_id"]}`);
  ...
  const output = prisma.result.upsert({
    where: {
...
async function getAllSeasons() {
  if (redisClient) {
    ...
    if (seasonLen > 0) {
      return (await redisClient.ZRANGE("seasons", 0, -1, {REV: true})).map(s => Number(s));
    }
  }
  ...
  for (const t of allTournaments) {
    if (seasons.length > 0 && seasons[seasons.length - 1] === t.tournament.year) {
      continue;
    }
  ...
  if (redisClient) {
    for (const season of seasons) {
      await redisClient.ZADD("seasons", {score: "1.0", value: season.toString()});
    }
  }
  return seasons;
}
...
async function getTournamentsBySeason(season) {
  if (redisClient) {
    ...
    const output = {};
    if (cached) {
      const tournamentList = await redisClient.ZRANGE(`seasons:${season}`, 0, -1, { REV: true });
      for (const tournament of tournamentList) {
        const tournamentExists = (await redisClient.HLEN(`seasons:${season}:${tournament}`)) === 6;
        if (!tournamentExists) {
          cached = false;
        }
        if (cached) {
          output[tournament] = await redisClient.HGETALL(`seasons:${season}:${tournament}`);
          output[tournament].official = output[tournament].official === "true";
          output[tournament].preliminary = output[tournament].preliminary === "true";
        }
      }
    }
    if (cached) {
  ...
  for (const r of tournaments) {
    output[r.duosmium_id] = {duosmium_id: r.duosmium_id, title: r.title, location: r.tournament.location, date: r.date, official: r.official, preliminary: r.preliminary};
  }
  if (redisClient) {
    for (const k of Object.keys(output)) {
      await redisClient.ZADD(`seasons:${season}`, {score: "1.0", value: k});
      await redisClient.HSET(`seasons:${season}:${k}`, "duosmium_id", output[k].duosmium_id);
      await redisClient.HSET(`seasons:${season}:${k}`, "title", output[k].title);
      await redisClient.HSET(`seasons:${season}:${k}`, "date", output[k].date);
      await redisClient.HSET(`seasons:${season}:${k}`, "location", output[k].location);
      await redisClient.HSET(`seasons:${season}:${k}`, "official", output[k].official.toString());
      await redisClient.HSET(`seasons:${season}:${k}`, "preliminary", output[k].preliminary.toString());
    }
  }
  return output;
}
